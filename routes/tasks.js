...
module.exports = (db) => {
  ...
  router.post("/new", (req, res) => {
    ...
    // const newObj = {"task" : req.body.content, "category" : ""};
    isMovie(task).then(
      (movie) => {
        if (movie['results'][0].title.toUpperCase() === task.toUpperCase()) {
    ...
    isRestaurant(task).then((restaurant) => {
      if ((restaurant.jsonBody.businesses[0] && restaurant.jsonBody.businesses[0].name.toUpperCase() === task.toUpperCase())) {
